...
$query_chamados  = "SELECT id_chamados, nome,email, nucleo, setor, tel, descr FROM chamados LIMIT 10"    ;
$result_chamado = $conn->prepare($query_chamados);
$result_chamado->execute();

$dados = "";
while ($row_usuario = $result_chamado->fetch(PDO::FETCH_ASSOC)) {
    extract($row_usuario);
    $dados .= "<tr>
                    <td>$id_chamados</td>
                    <td>$nome</td>
